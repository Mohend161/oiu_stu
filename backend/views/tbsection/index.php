...
/* @var $searchModel backend\models\TbsectionSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'Tbsections';
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="tbsection-index">

    <h1><?= Html::encode($this->title) ?></h1>

    <p>

            <?= Html::a('<span class=" btn btn-success">  add Tbsection</span>', ['create'], [
    ...
    <?php // echo $this->render('_search', ['model' => $searchModel]); ?>
       ...
       'class' => 'yii\grid\ActionColumn',
                ...
                'urlCreator' => function ($action, $model) {
                    if ($action === 'update') {
                        $url = Url::to(['tbsection/update', 'id' => $model->TySectionID]);
                        return $url;
                    }if ($action === 'view') {
                        $url = Url::to(['tbsection/view', 'id' => $model->TySectionID]);
                        return $url;
                    }if ($action === 'delete') {
                        $url = Url::to(['tbsection/delete', 'id' => $model->TySectionID]);
                        return $url;
                    }
                }
            ],
